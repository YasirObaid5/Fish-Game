...
(async () => {
  ...
  const score = await page.evaluate(() => { window.__ocean.place(-110, 7, 75); window.__ocean.spawn('pearl', -110, 7, 75); window.__ocean.step(.05); return window.__ocean.snapshot().run.score; });
  assert.equal(score, 5, 'Offline gameplay and point feedback run from the installed cache');
  await insideViewport(page, ['#stick', '#touch-boost', '#touch-rise', '#touch-dive', '#touch-skill', '#pause', '#sound', '#fullscreen']);
  ...
  const right = { ...center, x: center.x + 24, y: center.y - 24 };
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [right] });
  await page.waitForTimeout(220);
  const touchAfter = await state();
  assert.ok(touchAfter.swimmer.yaw > touchBefore.swimmer.yaw + .05, 'Touch joystick turns the heading');
  assert.ok(touchAfter.position[2] < touchBefore.position[2] - .05, 'Up-right joystick swims forward while turning');
  ...
  await page.evaluate(() => window.__ocean.step(.05));
  ...
  if (!process.env.GAME_URL) {
    ...
    await otherPage.evaluate(() => {
      window.__otherWindowSentinel = 'preserved'; window.__ocean.place(-110, 7, 75);
      window.__ocean.spawn('pearl', -110, 7, 75); window.__ocean.step(.05); window.__ocean.place(0, 10, 22);
    });
    await page.locator('#start').tap();
    await page.evaluate(() => { window.__ocean.place(-110, 7, 75); window.__ocean.spawn('pearl', -110, 7, 75); window.__ocean.step(.05); });
    revision = 'ready';
